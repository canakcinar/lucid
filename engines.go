...
// but "derive the timeout from what the engine has to do": wordlist size × 1/rate. Auto-
// derived timeout matches the workload; -engine-timeout on the CLI is the operator's override
// for exotic cases (slow VPN, paranoid rate limit).
//
// --------------------------------------------------------------------------------------------
...
//	deep     : lines × (1+exts) × depth  × 3 / rate (recursive; 3× hedge for recursion overhead)
//
// The (1+exts) factor accounts for the fact that -e php,json,txt,config,bak turns each word
// into 6 requests. The 3× hedge in deep mode absorbs ferox's recursion overhead: at -d 3
// each successful 200 hit re-fuzzes the whole wordlist inside the sub-directory, so the
// effective request count grows non-linearly with hit density. Floor 60s so a tiny wordlist
// doesn't derive a <5s deadline that a warm-up handshake alone can exceed.
//
...
// idle wall-time waste in batch mode).
func feroxBudget(cfg *Config, wordlist string) int {
	...
	case "deep":
		...
		// (an earlier build empirical). Depth cap at 3 keeps -d 5 from deriving a 3-hour deadline.
...
//	standard : -n (no recursion),    -x extensions.       Default. (an earlier build shape.)
//	deep     :    recursion at MaxDepth+1, -x extensions. Slowest.
//
//  background: default ferox re-fuzzes the whole wordlist inside every 2xx/3xx/401/403
// hit, so `common.txt -d 3` on a rich host executes ≈ hit_count^depth requests (measured
// 46,560 requests for example-a.tld = ~10× baseline, 26 minutes wall-clock). "Standard"
...
func runFerox(targetURL, wordlist string, cfg *Config) map[string]int {
	...
	// feroxBudget scales the deadline to the wordlist (rate + depth-aware); the 
...
// Budget is mode-aware because katana IS the recursion when ferox isn't (an earlier build+ standard,
// an earlier build+ fast). katana at -d 5 on a rich host can walk thousands of pages; a fixed
...
//  calibration on the two  hosts (raw katana, no lucid, -d 3 -rl 30):
//
//	example-a.tld       1404 URLs in 12.6s   (rich content, JS-heavy)
//	config-api.tld      32 URLs in 13.7s   (bare API + Okta redirect)
//
