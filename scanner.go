...
func (s *Scanner) verify(c cand, p Profile, dirAttempts, dirErrs *atomic.Int64) {
	...
	case r.Status == 200 && isConfigLike(c.url, r.CType, r.Body):
		...
		// service URL (AWS/Okta/Firebase/etc). The  sweep found `config.json` and
		// `manifest.json` under `example-target` carrying dev/prod AWS API Gateway endpoints —
	...
	if r.Status >= 200 && r.Status < 400 && r.Body != "" && f.Kind != "asset" {
		// Scan with values (): SecretMatches carries the actual matched substring so
