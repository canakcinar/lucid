...
// version is patched at build time via `-ldflags "-X main.version=an earlier build"`. Left as "dev"
...
// readTargetsFile parses -f: one URL per line, "#" comments, blank lines skipped.
// Returns an error the caller reports to stderr; empty files are a hard error too so a
// mistyped path doesn't silently no-op an operator's overnight sweep.
func readTargetsFile(path string) ([]string, error) {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("no targets in %s (all lines blank or commented)", path)
	}
	return out, nil
}

// slugForURL turns a URL into a filesystem-safe stem for per-target output paths.
// "https://api.example.tld/foo" → "api_example-target_com". Used by the -f multi-target
// path to derive `<out-dir>/<slug>.json` and `<har-dir>/<slug>.har`.
func slugForURL(raw string) string {
	...
	// Strip port and default replacements.
...
func main() {
	...
	// -t is the target flag (nmap mental model: -t = target, -f = target file).
	// Request timeout lives on -timeout so the -t namespace stays clean.
	flag.IntVar(&cfg.Timeout, "timeout", 15, "request timeout (seconds)")
	...
	outfile := flag.String("o", "", "write findings as JSON to this file (or a DIRECTORY when -f is used; each target writes <dir>/<slug>.json)")
	...
	targetFlag := flag.String("t", "", "single target URL (positional URL is also accepted)")
	targetsFile := flag.String("f", "", "file of target URLs, one per line (nmap-style -iL). '#' starts a comment, blank lines skipped")
	...
	// truncates on every target (the  sweep failure mode).
	...
	// Build the target list from -t / -f / positional. Rules:
	//   - -t and positional both name a single URL; either works, both together is a typo.
	//   - -f loads a URL list (nmap -iL shape). Any target source is fine on its own.
	//   - When -f is used, -o and -har are treated as DIRECTORIES: each target writes
	//     <dir>/<slug>.json and <dir>/<slug>.har so a batch doesn't overwrite itself.
	//   - Positional-only is also supported for the plain "lucid <url>" shape.
	var targets []string
	if *targetFlag != "" {
	...
	if *targetsFile != "" {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "targets:", err)
			os.Exit(1)
		}
	...
	if len(targets) == 0 {
		fmt.Fprintln(os.Stderr, "usage: lucid [flags] -t <url>   OR   lucid -f <targets-file>   OR   lucid [flags] <url>")
		flag.PrintDefaults()
		os.Exit(1)
	}
	if *targetFlag != "" && len(positional) > 0 {
		fmt.Fprintf(os.Stderr, "lucid: pass a target with EITHER -t OR positional, not both (got -t=%q and positional=%v)\n", *targetFlag, positional)
		os.Exit(1)
	}
	// When -f is used with a positional URL, that's fine — positional is appended after
	// the file's URLs and treated as another target in the batch.
	multi := len(targets) > 1
	// In multi-target mode -checkpoint / -resume / -diff are treated as DIRECTORIES —
	// each target reads/writes its own <dir>/<slug>.ckpt.jsonl (or .json), so the batch
	// can survive a mid-sweep crash and be resumed target-by-target. Pre-an earlier build lucid
	// refused these flags in multi mode to avoid state corruption; the directory shape
	// gives back the checkpoint feature without the corruption.
	//
	// If the operator passes a bare file for any of these in multi mode, we auto-treat
	// it as a directory (mkdir + per-target slug). Pre-existing FILE at that path is
	// caught by the isFile check below with a clear message.
	if multi {
		for _, spec := range []struct {
			...
			if st, err := os.Stat(*spec.path); err == nil && !st.IsDir() {
				fmt.Fprintf(os.Stderr, "lucid: %s=%s exists as a file; -f treats %s as a DIRECTORY (each target uses <dir>/<slug>.jsonl). Remove the file or pick another path.\n",
					spec.label, *spec.path, spec.label)
				os.Exit(1)
			}
			if err := os.MkdirAll(*spec.path, 0755); err != nil {
				fmt.Fprintf(os.Stderr, "lucid: cannot create %s directory %q: %v\n", spec.label, *spec.path, err)
				os.Exit(1)
			}
		}
	}
	// Make -o / -har directories when we're running multiple targets. If the operator
	// passed a file-shaped path like "out.json", we still use it as a directory prefix —
	// the individual outputs land under it. Better to create <path>/<slug>.json than to
	// silently overwrite the same file 24 times. If the path already exists AS A FILE,
	// tell the operator directly instead of leaking the underlying mkdir error.
	if multi {
		for _, spec := range []struct{ label, path string }{{"-o", *outfile}, {"-har", cfg.HAROutput}} {
			if spec.path == "" {
				continue
			}
			if st, err := os.Stat(spec.path); err == nil && !st.IsDir() {
				fmt.Fprintf(os.Stderr, "lucid: %s=%s exists as a file; -f needs a DIRECTORY (each target writes <dir>/<slug>.{json,har}). Remove the file or pick another path.\n", spec.label, spec.path)
				os.Exit(1)
			}
			if err := os.MkdirAll(spec.path, 0755); err != nil {
				fmt.Fprintf(os.Stderr, "lucid: cannot create %s directory %q: %v\n", spec.label, spec.path, err)
		...
		fmt.Printf("[*] batch: %d targets from -f + positional; per-target output in %s/, HAR in %s/\n",
			len(targets), safeDir(*outfile), safeDir(cfg.HAROutput))
	}
	// Save the operator-supplied templates. The per-target loop mutates these to per-slug
	// paths and restores them at end so a library caller using this Config after main()
	// doesn't inherit a stale per-target path.
	harTemplate := cfg.HAROutput
	outfileTemplate := *outfile
	ckptTemplate := cfg.CheckpointPath
	resumeTemplate := *resumeFile
	diffTemplate := *diffFile
	// Aggregate exit code across the batch: any target with a required-write failure or
	// bad URL flips it. A batch is only "success" when every scan wrote its artifacts.
	batchExit := 0
	for tIdx, raw := range targets {
		// Between targets: clear Truncated, PartialReason, reqCount, throttle. Without this
		// a target-2 scan starts with target-1's partial marker still set (misleading meta).
		if tIdx > 0 {
			cfg.ResetRuntime()
		}
		if multi {
			...
			if outfileTemplate != "" {
				*outfile = outfileTemplate + sep + slug + ".json"
			}
			if harTemplate != "" {
				cfg.HAROutput = harTemplate + sep + slug + ".har"
			}
			// Per-target checkpoint/resume/diff. Each target writes/reads its own file
			// so a batch survives a crash and can be resumed target-by-target.
			// -resume points at the SAME slug'd file (an operator re-running the batch
			// against the same -resume dir loads each target's prior findings). -diff
			// same: each target compared against its own baseline file, not a global one.
			if ckptTemplate != "" {
				cfg.CheckpointPath = ckptTemplate + sep + slug + ".ckpt.jsonl"
			}
			if resumeTemplate != "" {
				candidate := resumeTemplate + sep + slug + ".json"
				// A missing per-target resume file is fine — it's the first run for that
				// URL. Loud when the DIRECTORY is unreadable (already caught above), silent
				// when a specific slug hasn't been scanned yet.
				if _, err := os.Stat(candidate); err == nil {
					*resumeFile = candidate
				} else {
					*resumeFile = "" // no prior for this target
				}
			}
			if diffTemplate != "" {
				candidate := diffTemplate + sep + slug + ".json"
				if _, err := os.Stat(candidate); err == nil {
					*diffFile = candidate
				} else {
					*diffFile = ""
			...
			fmt.Printf("\n\x1b[36m[*] target %d/%d: %s\x1b[0m\n", tIdx+1, len(targets), raw)
		}
	...
	// Restore the operator-supplied templates so a library caller reusing this Config
	// after main() doesn't see the last target's slugged paths.
	cfg.HAROutput = harTemplate
	*outfile = outfileTemplate
	cfg.CheckpointPath = ckptTemplate
	*resumeFile = resumeTemplate
	*diffFile = diffTemplate
	if batchExit != 0 {
		os.Exit(batchExit)
	}
}

// safeDir returns the path if non-empty, "-" as a placeholder otherwise. Used only in
// the batch-start banner where we print "output in X/, HAR in Y/" and the operator may
// have omitted either flag.
