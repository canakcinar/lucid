...
bypass work, and lucid's own **SimHash cleanup core** — the one piece written from scratch —
...
| **Soft-404 cleanup + review** | **lucid SimHash** | **yes — the only original logic** |
...
positives — and no single tool cleans the *combined* output of brute + crawl + archive. lucid
...
## Usage
```bash
./lucid [flags] <url>            # flags may come before, after, or around the url
```
```bash
./lucid https://target.tld -w wordlist.txt
./lucid https://target.tld -w big.txt -e php,html -depth 3 -budget 50000 -o out.json
./lucid https://target.tld -w big.txt -b 'session=...' -x http://127.0.0.1:8080
./lucid https://target.tld -w big.txt -diff yesterday.json      # only-new monitoring
```
...
| `-c` | concurrent requests (lucid's cleanup fetches) | 20 |
| `-t` | request timeout (s) | 15 |
| `-delay` | base delay per request (ms); auto-backoff on 429/503 | 0 |
| `-budget` | max total **lucid cleanup** requests (0 = unlimited) | 0 |
| `-rate` | req/s cap passed to engines **and** lucid | 0 |
...
4. **Enrich** — 403/401 findings go to **nomore403** (bounded, rate-limited, ≤2 concurrent). Its
   result is tagged `unverified:` — lucid can't replay the exact winning request to SimHash-check
...
- **Catch-all/SPA detection** — if the homepage and a random path return near-identical 200s,
  lucid prints a warning: brute is low-signal, trust the crawler's JS endpoints. Responses that
...
- **WAF-pattern-block detection** — a 403 on a sensitive path may be the WAF matching the *name*
  (`.env`, `.git`), not proof the file exists. lucid fetches a same-shape decoy in the same dir; if
...
- `-har` writes a *separate* file (`-har out.har`); its schema is standard HAR 1.2. Request
  ...
  `[REDACTED by lucid]` so a shared HAR can't leak the operator's session.
...
- **Guaranteed coverage** — with `-rate`, lucid auto-raises `-engine-timeout` to `lines/rate` so
...
- **Rate control that reaches the loud part** — `-rate` is passed to feroxbuster/ffuf/katana/
  nomore403, not just lucid's own fetches, plus 429/503 auto-backoff on lucid's requests.
- **Caps** — `-max-candidates` bounds how many URLs lucid re-fetches (gau is capped at source);
  `-budget` caps lucid's own cleanup requests (note: it does not count requests made *inside* the
...
- lucid re-fetches each candidate once to compute its SimHash (the engines fetched it too), so a
  brute hit costs one extra request — the price of a uniform cleanup pass.
- On a pure SPA, brute finds little (every route is the same shell) — lucid now *detects and warns*
...
- **Bypass labels: `verified:` vs `unverified:`.** lucid replays the `headers` technique in-process
  ...
  nothing", not "unbypassable" — lucid's bypass *hit-rate is nomore403's payload coverage*.
...
lucid **cross-compiles** cleanly on macOS, Linux, and Windows — `GOOS=windows GOARCH=amd64 go build`
produces `lucid.exe` and `-audit` uses an in-process `httptest` mock target so it needs no
...
you run — lucid orchestrates them, it does not bundle them.
